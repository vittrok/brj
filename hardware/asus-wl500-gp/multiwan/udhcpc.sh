...
case "$1" in
    ...
    renew|bound)
        ...
        [ -n "$subnet" ] && NETMASK="netmask $subnet" || NETMASK="netmask `ipcalc -s -m $ip | awk '{print(substr($1,9))}'`"
        [ -n "$broadcast" ] && BROADCAST="broadcast $broadcast"|| BROADCAST="broadcast `ipcalc -s -b $ip $subnet | awk '{print(substr($1,11))}'`"
        ...
        [ "$proto" = "dhcp" ] && route add -host $gw dev $interface
