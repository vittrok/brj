...
case "$pptp_type" in
	static)
		/usr/local/sbin/wan-static.sh $1 && /usr/local/sbin/launch-pppd.sh $1 &
		;;
	dhcp)
		/usr/local/sbin/wan-dhcp.sh $1 && /usr/local/sbin/launch-pppd.sh $1 &
		;;
	vpn)
		/usr/local/sbin/launch-pppd.sh $1 &
		;;
	*)
		echo "Error: pptp type (static|dhcp|vpn) must be set in $1_pptp_type" && exit 1
		;;
esac
